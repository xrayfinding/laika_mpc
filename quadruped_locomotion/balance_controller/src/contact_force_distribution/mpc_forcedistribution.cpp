...
void CalculateAMat(const Vector3d& rpy, MatrixXd* a_mat_ptr) {
    ...
    a_mat(4, 10) = 1;
    a_mat(5, 11) = 1;
    a_mat(11, 12) = 1;//attention,not 12*12
}
...
void CalculateQpMats(const MatrixXd& a_exp, const MatrixXd& b_exp,
    ...
    //goaloxu:calculate the H in formular 0.5*U'HU+U'g
...
std::vector<double> ConvexMpc::ComputeContactForces(
    ...
    assert(foot_contact_states.size()== num_legs_);
    const double com_z =
    ...
    //std::cout << "com_z: " << com_z << std::endl;
    // Prepare the current and desired state vectors of length kStateDim *
    // planning_horizon.
    assert(com_velocity.size()== k3Dim);
    assert(com_angular_velocity.size() == k3Dim);
