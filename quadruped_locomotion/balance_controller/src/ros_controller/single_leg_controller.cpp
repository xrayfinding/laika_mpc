...
//        foot_positions[limb] = Vector(0.5,0.2,-0.45);
            ...
            ros::NodeHandle& node_handle)
  {
    ...
    //! WSHY: single leg controller from /home/mxr/catkin_laikago/src/quadruped_locomotion/single_leg_test/include/single_leg_test/model_test_header.hpp
    ...
    ROS_INFO("Single_leg controller to Get robot state handle");
    //! WSHY: get robot state handle
    robot_state_handle = hardware->getHandle("base_controller");

    ROS_INFO("Single_leg Controller initialized");
...
//MXR::NOTE: for dynamic set parameter in rqt_gui
...
//    std::cout<<"*********************"<<std::endl;
//    std::cout<<"leg_to_move.size()"<<leg_to_move.size()<<std::endl;
...
//        std::cout<<"foot_positions.at(limb).vector()  "<<foot_positions.at(limb).vector()<<std::endl;
//        std::cout<<"LinearVelocity(foot_velocities.at(limb).vector()  "<<LinearVelocity(foot_velocities.at(limb).vector())<<std::endl;
        ...
        if(real_contact_.at(limb)){
            //ROS_INFO("LEG IS SUPPORT!!!!");
          robot_state_->setSupportLeg(limb, true);}
        else{
            //ROS_INFO("LEG IS OUT OF SUPPORT!!!!");
            ...
            if(control_methods[i] == "end_position" || is_cartisian_motion_.at(limb))
              {
                ...
                //std::cout<< "foot_accelerations.at(limb).vector()" <<foot_accelerations.at(limb).vector()<<std::endl;
...
//    std::vector<double> joint_commands_;
    for(unsigned int i = 0;i<3;i++)
    {
      ...
      //std::cout<<joint_commands_[i]<<"  "<<joint_commands_[i+3]<<"  "<<joint_commands_[i+6]<<"  "<<joint_commands_[i+9]<<std::endl;
    }


    //MXR::NOTE cartesian means swing leg end_effector_trajectory
...
//                                        contact.contact_force.wrench.force.z);
  ...
  void SingleLegController::staticTFPublisher()
  {
    ...
    tfBoardcaster_.sendTransform(tf::StampedTransform(footprint_to_base, ros::Time::now(), "/foot_print", "/base"));
  ...
  void SingleLegController::robotStatePublisher()
  {
    ...
    robot_state_msg.lh_leg_joints.name[2] = "LH_KFE";
