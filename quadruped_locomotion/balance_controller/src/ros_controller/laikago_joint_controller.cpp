...
#include "balance_controller/ros_controler/laikago_joint_controller.hpp"
#include "balance_controller/ros_controler/laikago_control_tool.h"
...
namespace balance_controller {
    ...
    bool LaikagoJointController::init(hardware_interface::RobotStateInterface *robot, ros::NodeHandle &n)
    {
        ROS_WARN("LAIKAGO CONTROLLER INITING!!!!!!!!!");
        ...
        joint = robot->getHandle("base_controller");
    ...
    {
        ROS_WARN("LAIKAGO CONTROLLER UPDATING!!!!!!!");
        double currentPos, currentVel, calcTorque;
        lastCmd = *(command.readFromRT());


        servoCmd.pos = lastCmd.position;
        positionLimits(servoCmd.pos);//try to limit the joint_position in the safe_limit
        ...
        joint.setCommand(calcTorque);//effort control
...
PLUGINLIB_EXPORT_CLASS(balance_controller::LaikagoJointController, controller_interface::ControllerBase);
