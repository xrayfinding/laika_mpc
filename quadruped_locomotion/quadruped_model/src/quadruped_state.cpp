...
//  std::cout<<"Constructor QuadrupedState"<<std::endl;
  limb_mass_ = limb_mass({
                         {LimbEnum::LF_LEG, 2.925},
                         {LimbEnum::RF_LEG, 2.925},
                         {LimbEnum::RH_LEG, 2.925},
                         {LimbEnum::LH_LEG, 2.925},
                       });
...
//    std::cout<<"Initialize QuadrupedState"<<std::endl;
    ...
    setPoseBaseToWorld(Pose(Position(0,0,0), RotationQuaternion()));
    joint_positions_ << 0,1.2,-2.7,0,1.2,-2.7,0,1.2,-2.7,0,1.2,-2.7;
    setCurrentLimbJoints(joint_positions_);
...
//  return getPositionBaseToLegMassCenterInBaseFrame(limb, getJointPositionFeedbackForLimb(limb)) - CoM_in_base_;
  switch (limb) {
    case LimbEnum::LF_LEG:
      return Position(0.21935, 0.087, 0.0) - CoM_in_base_;
