  ...
  nodehandle_(nodehandle)
{
  ...
  ui->Controllers->setTabText(5, "MPC");
  ...
  my_switchControllerClient_ = nodehandle_.serviceClient<controller_manager_msgs::SwitchController>
          ("/mxr/switch_controller", false);
...
//  std::cout<<list_controllers.response.controller[0].name<<std::endl;
  ...
  for(auto& controller:list_controllers.response.controller)
    {
      ...
      if(controller.name == "base_mpc_controller" and controller.state == "running"){
          current_controller = "base_mpc_controller";
          break;
      }
  ...
  if(tab_name == "Balance")
    {
    ...
    }else if (tab_name == "MPC" ) {
      e_stop_msg.data = false;
      eStopPublisher_.publish(e_stop_msg);
      std::cout<<"MPC"<<std::endl;
      controller_switch.request.start_controllers.push_back("base_mpc_controller");
      controller_switch.request.stop_controllers.push_back(current_controller.c_str());
      controller_switch.request.strictness = controller_switch.request.STRICT;
      switchControllerClient_.call(controller_switch.request, controller_switch.response);
      my_switchControllerClient_.call(controller_switch.request, controller_switch.response);
      ...
      if(controller_switch.response.ok && control_method.response.result)
        {
          control_method_ = MPC;
          displayOutputInfos("green", "Switch to MPC Controller");
        }
...
void rqt_control_panel_plugin_widget::on_trotButton_3_clicked()
{
  std_srvs::SetBool trot_switch;
  trot_switch.request.data = true;
  trotSwitchClient_.call(trot_switch.request, trot_switch.response);
...
void rqt_control_panel_plugin_widget::on_stopBotton_3_clicked()
{
  std_srvs::SetBool gait_stop_switch;
  gait_stop_switch.request.data = false;
  trotSwitchClient_.call(gait_stop_switch.request, gait_stop_switch.response);
...
void rqt_control_panel_plugin_widget::on_setJointPositionButton_clicked()
{
  ...
  std_srvs::SetBool init_process_start;
  init_process_start.request.data = true;
  laikago_position_init.call(init_process_start.request,init_process_start.response);
  if(init_process_start.response.success){
      displayOutputInfos("blue","Position init....");
  }
...
void rqt_control_panel_plugin_widget::on_stopPositionButton_clicked()
{
  ...
  std_srvs::SetBool init_process_stop;
  init_process_stop.request.data = true;
  laikago_position_init_stop.call(init_process_stop.request,init_process_stop.response);
  if(init_process_stop.response.success){
      displayOutputInfos("blue","Position init stop....");
  }
}
...
void rqt_control_panel_plugin_widget::on_setVelocityBotton_3_clicked()
{
  geometry_msgs::Twist cmd_vel;
  cmd_vel.linear.x = ui->desired_vx->value();
  cmd_vel.linear.y = ui->desired_vy->value();
...
{
  ui->lf_joint_positon_1->setValue(0.2);
  ui->lf_joint_positon_2->setValue(1.8);
  ui->lf_joint_positon_3->setValue(-2.7);

  ui->rf_joint_positon_1->setValue(-0.2);
  ui->rf_joint_positon_2->setValue(1.8);
  ui->rf_joint_positon_3->setValue(-2.7);

  ui->rh_joint_positon_1->setValue(-0.2);
  ui->rh_joint_positon_2->setValue(1.8);
  ui->rh_joint_positon_3->setValue(-2.7);

  ui->lh_joint_positon_1->setValue(0.2);
  ui->lh_joint_positon_2->setValue(1.8);
  ui->lh_joint_positon_3->setValue(-2.7);
}
