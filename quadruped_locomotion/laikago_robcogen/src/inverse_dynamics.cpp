...
const laikago::rcg::InverseDynamics::ExtForces
laikago::rcg::InverseDynamics::zeroExtForces(Force::Zero());

laikago::rcg::InverseDynamics::InverseDynamics(InertiaProperties& inertia, MotionTransforms& transforms) :
...
{
#ifndef EIGEN_NO_DEBUG
    std::cout << "Robot laikago, InverseDynamics::InverseDynamics()" << std::endl;
...
void laikago::rcg::InverseDynamics::id(
    ...
    const ExtForces& fext)
{
    ...
    motionCrossProductMx<Scalar>(RF_HIP_v, vcross);
    
    RF_HIP_a = (vcross.col(iit::rbd::AZ) * qd(RF_HAA));
    RF_HIP_a(iit::rbd::AZ) += qdd(RF_HAA);
    
    RF_HIP_f = RF_HIP_I * RF_HIP_a + vxIv(RF_HIP_v, RF_HIP_I);
    ...
    motionCrossProductMx<Scalar>(LF_HIP_v, vcross);
    
    LF_HIP_a = (vcross.col(iit::rbd::AZ) * qd(LF_HAA));
    LF_HIP_a(iit::rbd::AZ) += qdd(LF_HAA);
    
    LF_HIP_f = LF_HIP_I * LF_HIP_a + vxIv(LF_HIP_v, LF_HIP_I);
    ...
    motionCrossProductMx<Scalar>(RH_HIP_v, vcross);
    
    RH_HIP_a = (vcross.col(iit::rbd::AZ) * qd(RH_HAA));
    RH_HIP_a(iit::rbd::AZ) += qdd(RH_HAA);
    
    RH_HIP_f = RH_HIP_I * RH_HIP_a + vxIv(RH_HIP_v, RH_HIP_I);
    ...
    motionCrossProductMx<Scalar>(LH_HIP_v, vcross);
    
    LH_HIP_a = (vcross.col(iit::rbd::AZ) * qd(LH_HAA));
    LH_HIP_a(iit::rbd::AZ) += qdd(LH_HAA);
    
    LH_HIP_f = LH_HIP_I * LH_HIP_a + vxIv(LH_HIP_v, LH_HIP_I);
    ...
    iit::rbd::transformInertia<Scalar>(LH_SHANK_Ic, (xm->fr_LH_SHANK_X_fr_LH_THIGH).transpose(), Ic_spare);
    LH_THIGH_Ic += Ic_spare;
    LH_THIGH_f = LH_THIGH_f + (xm->fr_LH_SHANK_X_fr_LH_THIGH).transpose() * LH_SHANK_f;
    
    iit::rbd::transformInertia<Scalar>(LH_THIGH_Ic, (xm->fr_LH_THIGH_X_fr_LH_HIP).transpose(), Ic_spare);
    LH_HIP_Ic += Ic_spare;
    LH_HIP_f = LH_HIP_f + (xm->fr_LH_THIGH_X_fr_LH_HIP).transpose() * LH_THIGH_f;
    
    iit::rbd::transformInertia<Scalar>(LH_HIP_Ic, (xm->fr_LH_HIP_X_fr_base).transpose(), Ic_spare);
    base_Ic += Ic_spare;
    base_f = base_f + (xm->fr_LH_HIP_X_fr_base).transpose() * LH_HIP_f;
    
    iit::rbd::transformInertia<Scalar>(RH_SHANK_Ic, (xm->fr_RH_SHANK_X_fr_RH_THIGH).transpose(), Ic_spare);
    RH_THIGH_Ic += Ic_spare;
    RH_THIGH_f = RH_THIGH_f + (xm->fr_RH_SHANK_X_fr_RH_THIGH).transpose() * RH_SHANK_f;
    
    iit::rbd::transformInertia<Scalar>(RH_THIGH_Ic, (xm->fr_RH_THIGH_X_fr_RH_HIP).transpose(), Ic_spare);
    RH_HIP_Ic += Ic_spare;
    RH_HIP_f = RH_HIP_f + (xm->fr_RH_THIGH_X_fr_RH_HIP).transpose() * RH_THIGH_f;
    
    iit::rbd::transformInertia<Scalar>(RH_HIP_Ic, (xm->fr_RH_HIP_X_fr_base).transpose(), Ic_spare);
    base_Ic += Ic_spare;
    base_f = base_f + (xm->fr_RH_HIP_X_fr_base).transpose() * RH_HIP_f;
    
    iit::rbd::transformInertia<Scalar>(LF_SHANK_Ic, (xm->fr_LF_SHANK_X_fr_LF_THIGH).transpose(), Ic_spare);
    LF_THIGH_Ic += Ic_spare;
    LF_THIGH_f = LF_THIGH_f + (xm->fr_LF_SHANK_X_fr_LF_THIGH).transpose() * LF_SHANK_f;
    
    iit::rbd::transformInertia<Scalar>(LF_THIGH_Ic, (xm->fr_LF_THIGH_X_fr_LF_HIP).transpose(), Ic_spare);
    LF_HIP_Ic += Ic_spare;
    LF_HIP_f = LF_HIP_f + (xm->fr_LF_THIGH_X_fr_LF_HIP).transpose() * LF_THIGH_f;
    
    iit::rbd::transformInertia<Scalar>(LF_HIP_Ic, (xm->fr_LF_HIP_X_fr_base).transpose(), Ic_spare);
    base_Ic += Ic_spare;
    base_f = base_f + (xm->fr_LF_HIP_X_fr_base).transpose() * LF_HIP_f;
    
    iit::rbd::transformInertia<Scalar>(RF_SHANK_Ic, (xm->fr_RF_SHANK_X_fr_RF_THIGH).transpose(), Ic_spare);
    RF_THIGH_Ic += Ic_spare;
    RF_THIGH_f = RF_THIGH_f + (xm->fr_RF_SHANK_X_fr_RF_THIGH).transpose() * RF_SHANK_f;
    
    iit::rbd::transformInertia<Scalar>(RF_THIGH_Ic, (xm->fr_RF_THIGH_X_fr_RF_HIP).transpose(), Ic_spare);
    RF_HIP_Ic += Ic_spare;
    RF_HIP_f = RF_HIP_f + (xm->fr_RF_THIGH_X_fr_RF_HIP).transpose() * RF_THIGH_f;
    
    iit::rbd::transformInertia<Scalar>(RF_HIP_Ic, (xm->fr_RF_HIP_X_fr_base).transpose(), Ic_spare);
    base_Ic += Ic_spare;
...
void laikago::rcg::InverseDynamics::G_terms_fully_actuated(
...
void laikago::rcg::InverseDynamics::C_terms_fully_actuated(
    Force& baseWrench, JointState& jForces,
...
void laikago::rcg::InverseDynamics::id_fully_actuated(
        Force& baseWrench, JointState& jForces,
        ...
        const JointState& qd, const JointState& qdd, const ExtForces& fext)
{
    ...
    RF_HIP_f = RF_HIP_I * RF_HIP_a + vxIv(RF_HIP_v, RF_HIP_I) - fext[RF_HIP];
    ...
    LF_HIP_f = LF_HIP_I * LF_HIP_a + vxIv(LF_HIP_v, LF_HIP_I) - fext[LF_HIP];
    ...
    RH_HIP_f = RH_HIP_I * RH_HIP_a + vxIv(RH_HIP_v, RH_HIP_I) - fext[RH_HIP];
    ...
    LH_HIP_f = LH_HIP_I * LH_HIP_a + vxIv(LH_HIP_v, LH_HIP_I) - fext[LH_HIP];
...
void laikago::rcg::InverseDynamics::secondPass_fullyActuated(JointState& jForces)
{
    // Link 'LH_SHANK'
