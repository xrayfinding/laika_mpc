...
const laikago::rcg::ForwardDynamics::ExtForces
    laikago::rcg::ForwardDynamics::zeroExtForces(Force::Zero());

laikago::rcg::ForwardDynamics::ForwardDynamics(InertiaProperties& inertia, MotionTransforms& transforms) :
...
void laikago::rcg::ForwardDynamics::fd(
    JointState& qdd,
    Acceleration& base_a,
    ...
    const ExtForces& fext/* = zeroExtForces */)
{
    ...
    RF_HIP_c = vcross.col(AZ) * qd(RF_HAA);
    
    //  - The bias force term:
    RF_HIP_p += vxIv(RF_HIP_v, RF_HIP_AI);
    ...
    LF_HIP_c = vcross.col(AZ) * qd(LF_HAA);
    
    //  - The bias force term:
    LF_HIP_p += vxIv(LF_HIP_v, LF_HIP_AI);
    ...
    RH_HIP_c = vcross.col(AZ) * qd(RH_HAA);
    
    //  - The bias force term:
    RH_HIP_p += vxIv(RH_HIP_v, RH_HIP_AI);
    ...
    LH_HIP_c = vcross.col(AZ) * qd(LH_HAA);
    
    //  - The bias force term:
    LH_HIP_p += vxIv(LH_HIP_v, LH_HIP_AI);
    ...
    compute_Ia_revolute(LH_HIP_AI, LH_HIP_U, LH_HIP_D, Ia_r);  // same as: Ia_r = LH_HIP_AI - LH_HIP_U/LH_HIP_D * LH_HIP_U.transpose();
    ...
    compute_Ia_revolute(RH_HIP_AI, RH_HIP_U, RH_HIP_D, Ia_r);  // same as: Ia_r = RH_HIP_AI - RH_HIP_U/RH_HIP_D * RH_HIP_U.transpose();
    ...
    compute_Ia_revolute(LF_HIP_AI, LF_HIP_U, LF_HIP_D, Ia_r);  // same as: Ia_r = LF_HIP_AI - LF_HIP_U/LF_HIP_D * LF_HIP_U.transpose();
    ...
    compute_Ia_revolute(RF_HIP_AI, RF_HIP_U, RF_HIP_D, Ia_r);  // same as: Ia_r = RF_HIP_AI - RF_HIP_U/RF_HIP_D * RF_HIP_U.transpose();
