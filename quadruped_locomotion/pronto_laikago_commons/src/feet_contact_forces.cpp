...
#include "laikago/rcg/dynamics_parameters.h"
using namespace iit;
...
//    std::cout <<"tau_leg" << tau_leg << std::endl;
//    std::cout <<"h_leg" << h_leg << std::endl;
...
bool FeetContactForces::getTorqueFromIDyn(const JointState& q,
                       const JointState& qd,
                       JointState& tau,
                       const Quaterniond& orient,
                       const LegID& leg,
    ...
    Eigen::Matrix3d R = pronto::commons::quatToRotMat(orient);
    gravity_base.segment(rbd::LX, 3) = R * gravity_world.segment(rbd::LX, 3);
    base_acceleration.segment(rbd::LX, 3) = xdd;
    base_acceleration.segment(rbd::AX, 3) = omegad;
    base_twist.segment(rbd::AX, 3) = omega;
    base_twist.segment(rbd::LX, 3) = xd;
    rbd::ForceVector h_base;
    JointState h_joints;
    jsim_(q);
    inverse_dynamics_.id_fully_actuated(h_base,h_joints,gravity_base,base_twist,base_acceleration,q,qd,qdd);
    Eigen::Vector3d h_leg = pronto::quadruped::getLegJointState(LegID(leg), h_joints);
    tau.segment(leg*3, 3) = h_leg - (foot_jacobian.transpose())*foot_grf;
