...
//                                  std::bind(&GaitGenerateClient::doneCallback, this, std::placeholders::_1, std::placeholders::_2));
  ...
  void GaitGenerateClient::initialize()
  {
    ...
    hip_to_foot_in_base.emplace(LimbEnum::LF_LEG, Position(0,0,-0.38));
    hip_to_foot_in_base.emplace(LimbEnum::RF_LEG, Position(0,0,-0.38));
    hip_to_foot_in_base.emplace(LimbEnum::RH_LEG, Position(0,0,-0.38));
    hip_to_foot_in_base.emplace(LimbEnum::LH_LEG, Position(0,0,-0.38));
...
//YG：动态行走落脚点生成公式，footprint是baselink坐标系的一个投影，下面的代码里面有一部分是没用的
            ...
            if(pace_flag || crawl_flag)
              {
// YG:静态爬行步态速度系数不准，是大致取的，不准，根据时序取，根据摆动周期来计算
                displace_in_footprint = Position(0.34*t_stance_*desired_vel2D);
//                std::cout<<"**************************"<<std::endl;
//                std::cout<<displace_in_footprint<<std::endl;
//                std::cout<<"**************************"<<std::endl;
...
//            std::cout<<"$$$$$$$$$$$$$$$$$$"<<std::endl;
//            std::cout<<"target_in_baselink     "<<target_in_baselink<<std::endl;
//            std::cout<<"displace_in_baselink   "<<displace_in_baselink<<std::endl;
//            std::cout<<"hip_in_baselink    "<<hip_in_baselink<<std::endl;
//            std::cout<<"$$$$$$$$$$$$$$$$$$"<<std::endl;
//            if(crawl_flag)
...
//                orinetationFootprintToWorld.rotate(target_in_footprint + Position(t_stance_*desired_vel2D));
            ...
            if(crawl_flag || pace_flag)
              {
                frame = "odom";
                //ROS_ERROR("COMINNG TO THR CRAWL_FLAG BRANCH!!!");
// YG：计算由角速度产生的偏置
...
//                std::cout<<"$$$$$$$$$$$$$$$$"<<std::endl;
//                std::cout<<"robot_state_.getPositionBaseToHipInBaseFrame(limb)   "<<robot_state_.getPositionBaseToHipInBaseFrame(limb)<<std::endl;
//                std::cout<<"hip_dispacement.at(limb)   "<<hip_dispacement.at(limb)<<std::endl;
//                std::cout<<"$$$$$$$$$$$$$$$$"<<std::endl;
...
//MXR::NOTE: generate foothfold important!!!!!!!!!!!!!!!!
                target_in_odom = next_pose_.getPosition() + next_pose_.getRotation().rotate(hip_in_baselink + displace_of_angular);

//                std::cout<<"$$$$$$$$$$$$$$$$"<<std::endl;
//                std::cout<<"hip_in_baselink  "<<hip_in_baselink<<std::endl;
//                //std::cout<<"displace_of_angular   "<<displace_of_angular<<std::endl;
//                //std::cout<<"next_pose_.getPosition()   "<<next_pose_.getPosition()<<std::endl;
//                std::cout<<"next_pose_.getRotation().rotate(hip_in_baselink + displace_of_angular) "<<next_pose_.getRotation().rotate(hip_in_baselink + displace_of_angular);
//                std::cout<<"$$$$$$$$$$$$$$$$"<<std::endl;
...
//                std::cout<<"#########################"<<std::endl;
//                std::cout<<"if(frame==base)"<<std::endl;
//                std::cout<<"stance to reach: "<<stance_to_reach[limb]<<std::endl;
//                std::cout<<"#########################"<<std::endl;
                if(use_terrian_map)
                  {
...
//                                                   footstep_msg_.target.point);
    //            footstep_msg_.target.point.z = 0;
                footstep_msg_.target.header.frame_id = "base";
...
//    ROS_WARN_STREAM("Desired Velocity :"<<desired_linear_velocity_<<std::endl);
    visualization_msgs::Marker base_marker;
    visualization_msgs::MarkerArray com_proj_markers;
// YG：在对角步态时，期望速度，角速度等于实际给的指令，在其他步态期望速度角速度等于0，在pace或者Crawl，身体不是一直往前动，身体一直在调整，速度就不用期望速度，用规划器去更新
// 只控制质心位置，在pace和Crawl，质心速度是不一定的
    if(trot_flag)
      {
        ignore_vd = false;
...
****************/
// YG：这一块主要是计算权重,vitual support region,基于权重计算质心轨迹
...
//YG：身体动态行走姿态适应，根据四条腿的位置构成一个平面，算平面的姿态作为身体的姿态来适应斜坡
...
****************/
// YG：如果并非爬行，把Step_msgs_放到steps里在send
...
// YG：对于Crawl来说，没有根据时间切换，根据Step来切换，第一次迈步的Step结束后，收到结束信号，再发一个baseauto，移一下身体位置
// 即是抬腿->移身体->抬腿->移身体,根据单个Step来切换的
