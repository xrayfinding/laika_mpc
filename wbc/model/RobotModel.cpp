...
#define THIS_COM_XRAY "/home/glx/Documents/DataTesting/Rbdl_examples_code-master/rbdl_using/urdf/"
...
RobotModel::RobotModel(){
  ...
  if (!Addons::URDFReadFromFile (
          THIS_COM_XRAY"laikago.urdf", model_, true, false)) {
    std::cerr << "Error loading model Laikago.urdf" << std::endl;
    abort();
  } else{
      std::cout << "Loading urdf success!!!"<<"\n\n";
  }

  dyn_model_ = new DynModel(model_);
  kin_model_ = new KinModel(model_);

  printf("[Laikago Model] Contructed\n");
}
